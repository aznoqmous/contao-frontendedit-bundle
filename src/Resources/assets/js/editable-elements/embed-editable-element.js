...
export default class EmbedEditableElement extends EditableElement {
    ...
    bindDrag(){
        ...
        this.element.addEventListener('dragstart', (e) => {
            ...
            dragging = true
            next = this.getNextSiblingEditableElement()
            prev = this.getPreviousSiblingEditableElement()
            box = this.element.getBoundingClientRect()
            nextBox = next ? next.element.getBoundingClientRect() : null
            prevBox = prev ? prev.element.getBoundingClientRect() : null
        })

        this.element.addEventListener('drag', (e) => {
            if(e.clientY < 0) return null
            if (!dragging) return null

            if (
                (box && nextBox)
                && ( /* HORIZONTAL DRAG */
                    box.left < nextBox.left &&
                    e.pageX > nextBox.left
                ) || ( /* VERTICAL DRAG */
                    box.top < nextBox.top &&
                    e.clientY > nextBox.top
                )){
                moveDown = true
            }
            else {
            ...
            if ((box && prevBox)
                && ( /* HORIZONTAL DRAG */
                box.left > prevBox.left &&
                e.pageX < prevBox.left
            ) || (/* VERTICAL DRAG */
                box.top > prevBox.top &&
                e.clientY < prevBox.bottom
            )){
                moveUp = true
            }
            else {
    ...
    moveElementUp() {
        let previousElement = this.getPreviousSiblingEditableElement()
    ...
    moveElementDown() {
        let nextElement = this.getNextSiblingEditableElement()
    ...
    getPreviousSiblingEditableElement(index = 1, dodgeNewContent = true) {
        let sameTypeElements = this.getSameTypeElements(dodgeNewContent).filter(el => el.parentElement === this.element.parentElement)
        let previousElement = sameTypeElements ? sameTypeElements[sameTypeElements.indexOf(this.element) - index] : null
        return previousElement ? previousElement.editable : null
    }

    getNextSiblingEditableElement(index = 1, dodgeNewContent = true) {
        let sameTypeElements = this.getSameTypeElements(dodgeNewContent).filter(el => el.parentElement === this.element.parentElement)
        let nextElement = sameTypeElements ? sameTypeElements[sameTypeElements.indexOf(this.element) + index] : null
        return nextElement ? nextElement.editable : null
    }
